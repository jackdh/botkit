...
function SocketBot(configuration) {
    ...
    if (!configuration.socketPort) {
        console.log("Defaulting to socket.io port 4000");
        configuration.socketPort = 4000;
    }
    
    io.attach(configuration.socketPort);

    controller.excludeFromConversations = ['hello', 'welcome_back', 'reconnect'];

    controller.openSocketServer = function (server) {
        
        console.log("Socket.io online at port: " + configuration.socketPort);
    ...
    /**
     * Label incoming messages as message_received
     */
    ...
    controller.defineBot(function (botkit, config) {
        ...
        bot.startTyping = function () {
            if (bot.connected) {
                bot.ws.emit("message", {isTyping: 'typing'});
            }
        };
            ...
            for (var t = 0; t < botkit.tasks.length; t++) {
                for (var c = 0; c < botkit.tasks[t].convos.length; c++) {
                    if (
                        ...
                        controller.excludeFromConversations.indexOf(message.type) == -1 // this type of message should not be included
                    ) {
